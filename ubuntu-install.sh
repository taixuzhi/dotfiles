...
function install_basic(){
    ...
    sudo apt-get install -y ssh openssl openssh-client openssh-server
...
function installemacs(){
    EMACS_VER=25.3
    cd ${SWDIR}
    sudo apt-get install -y build-essential texinfo libx11-dev libxpm-dev libgif-dev libxaw7-dev libjpeg-dev libpng12-dev libtiff4-dev libncurses5-dev xsel magit
...
function installerlang(){
    ...
    wget http://erlang.org/download/otp_doc_man_19.3.tar.gz
    tar xvf otp_src_19.3.tar.gz
    sudo apt-get install -y build-essential autoconf m4 libncurses5-dev libwxgtk2.8-dev libgl1-mesa-dev libglu1-mesa-dev libpng3 libssh-dev unixodbc-dev
    cd otp_src_19.3 && ./configure && make && sudo make install
    cd ${SWDIR}
    tar xvf otp_doc_man_19.3.tar.gz
